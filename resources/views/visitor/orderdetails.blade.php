        ...
        <div class="card">
            <div class="card-body d-flex">
                <div class="col-lg-4 col-sm-6">
                    ...
                    <p class="my-0"> {{ $order->shippingadd->landmark->landmark_eng }} | {{ $order->shippingadd->pincode }}
                    </p>
                </div>
                <div class="col-lg-4 col-sm-6">
                ...
                @foreach ($order->orderDetails as $orderData)
                    <div class="row border-top py-3">
                        <div class="product-image d-flex align-items-center justify-content-center col-lg-2">
                            <img src="{{ asset( $orderData->product->image) }}" alt=""
                                class="" style="max-height: 150px">
                        </div>
                        <div class="col-lg-8 px-2">
                            <div class="product-details">
                                <h6>{{ $orderData->product->productName }}</h6>
                                <p>{!! $orderData->product->productDescription !!}</p>
                                <p class="m-0">{{ $orderData->Unit->unitMaster->unit }} ({{ $orderData->qty }} qty)</p>
                                <p>₹ {{ $orderData->total }}</p>
                            </div>
                            <div class="mt-auto">
